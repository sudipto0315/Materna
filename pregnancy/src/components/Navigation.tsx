...
const Navigation: React.FC<NavigationProps> = ({ className }) => {
  ...
  const navItems = [
    ...
    { name: "Upload Reports", path: "/upload-reports", enabled: isRegistered, icon: <Upload size={16} className="mr-2" /> },
    { name: "Health Questionnaire", path: "/questionnaire", enabled: isRegistered, icon: <Clipboard size={16} className="mr-2" /> },
    { name: "View Reports", path: "/view-reports", enabled: isRegistered, icon: <FileText size={16} className="mr-2" /> },
    { name: "Summary Dashboard", path: "/dashboard", enabled: isRegistered, icon: <LayoutDashboard size={16} className="mr-2" /> },
  ...
  return (
    <div className={cn("relative bg-gray-50 border-b border-gray-200 p-4 flex items-center justify-between", className)}>
      <div className="flex items-center gap-4">
        ...
        <Link to={isRegistered ? "/dashboard" : "/register"} className="flex items-center gap-2 hover:text-materna-700 transition-colors">
